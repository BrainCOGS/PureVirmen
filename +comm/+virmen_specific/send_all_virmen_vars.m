...
comm.tcp.send_binary_mat_file(tcp_client, codes_files);
...
for i=1:length(files2send)
    ...
    if next_file == 2
        ...
        comm.tcp.set_tcp_bytes(tcp_client, uint8(codes_files.(files2send{i})));
        pause(0.05);
        %Send next file
        comm.tcp.send_binary_mat_file(tcp_client, virmen_structure.(files2send{i}));
    end
