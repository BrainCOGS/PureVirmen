function code = alvaro_poisson_blocks
...
function vr = initializationCodeFun(vr)
  ...
  vr.size_colormap = 256;
  vr.colormap_maze  = generateColorMap(vr.size_colormap);  
  ...
  switch vr.state           % ... take action depending on the simulation state
    ...
    case BehavioralState.WithinTrial
      ...
      elseif vr.iCueEntry < 1 && ~isPastCrossing(vr.cross_cue, vr.position)
          ...
          %ALS Fading effect, if in "green context" 
          if (vr.context == 2)
              %get index of colormap based on distance traveled
              dist_start = (vr.lStart) + vr.position(2);
              if dist_start > 0
              length_start = vr.lStart + vr.cross_cue.crossing;
              color_idx = round(dist_start*(vr.size_colormap-1)/length_start)+1;
              else
                  color_idx = 1;
              end
              %Change background
              vr.worlds{vr.currentWorld}.backgroundColor  = ...
                  vr.colormap_maze(color_idx,:);
          end
    ...
    case BehavioralState.DuringReward
      ...
      if ~RigParameters.hasDAQ && RigParameters.simulationMode
        ...
        elseif vr.choice == vr.wrongChoice
          %vr.worlds{vr.currentWorld}.backgroundColor  = [1 1 0] * 0.8;
        else
...
function vr = terminationCodeFun(vr)
  ...
  % write to datajoint database
  try writeTrainingDataToDatabase(log,vr); catch; warning('Problem writing to database; Please contact database administrator'); end

end

% Generate colormap for "fading" effects
function colormap_maze = generateColorMap(size_colormap)

% start parameters for colormap
start_color = [0.8 1 0.2];
end_color = [0 0 0];

%linspace for interpolation
xq = 1:size_colormap;
% start and end points of interpolation
X = [1 size_colormap];

%start and end r g b colors
Y = zeros(1,2);
% colormap variable initialization
colormap_maze = zeros(size_colormap,3);
for i=1:length(start_color)
    Y(1) =  start_color(i);
    Y(2) =  end_color(i);
    
    colormap_maze(:,i) = interp1(X,Y,xq);
end
...
function vr = computeWorld(vr)
  ...
  %ALS, state for the turnhint towers 0 = not reversed, 1 = reversed
  vr.tri_turnHintState      = false;
...
function vr = initializeTrialWorld(vr)
  ...
  if isfield(vr.mazes(vr.mazeID).variable,'context')
    [vr, mazeChanged]         = decideMazeAdvancementContexts(vr, vr.numMazesInProtocol);
    [vr.context, vr.stage]    = getContextStage(vr.mazes, vr.mazeID); 
  else
...
function [vr, nonTrivial] = drawCueSequence(vr)
  ...
  % representation
  antitrial = (rand() < vr.antiFraction);
  nominalSide               = 1 + antitrial;
  ...
  %If this trial is     anti and cuehint are not flipped or ...
  %If this trial is not anti abd cuehint are     filpped
%   if ((~vr.tri_turnHintState && antitrial) || (vr.tri_turnHintState && ~antitrial))
%       %Flip cuehints
%       vr.tri_turnHint     = flipud(vr.tri_turnHint);
%       %Set cuehints flipped state to inverse state
%       vr.tri_turnHintState= not(vr.tri_turnHintState);
%   end
...
function vr = setupTrials(vr, shaping)
  ...
  cfg.blockData         = { 'mazeID', 'mainMazeID', 'motionBlurRange', 'iterStr', 'shapingProtocol'             ...
   ...
   %ALS_correct, save context and stage data if defined
   if isfield(mazes,'context')
       cfg.blockData = [cfg.blockData {'context', 'stage'}];
   end
